...
def process_image(img_path):
    try:
        ...
        small_array = cv2.resize(img_array, (64, 64), interpolation=cv2.INTER_AREA)
        return (new_array, small_array)
    except Exception as e:
        return None

# Create training data using multiprocessing
def create_training_data_parallel(datadir):
    img_paths = [os.path.join(datadir, img) for img in os.listdir(datadir)]
    with mp.Pool(processes=mp.cpu_count()) as pool:
        results = pool.map(process_image, img_paths)
    results = [res for res in results if res is not None]
    array, array_small = zip(*results)
    return np.array(array), np.array(array_small)
...
def train_model_parallel(X_train, y_train, generator, discriminator, gan_model, vgg):
    ...
    for e in range(epochs):
        ...
        with mp.Pool(processes=8) as pool:
            results = pool.map(train_batch, train_dataset)
        
        # Aggregate the losses
        g_losses, d_losses_gen, d_losses_real = zip(*results)
        g_loss = np.mean(g_losses)
        d_loss = 0.5 * (np.mean(d_losses_gen) + np.mean(d_losses_real))
...
if __name__=='__main__':
    ...
    array, array_small = create_training_data_parallel(datadir)
    X = array.reshape(-1, 128, 128, 3)
    Xs = array_small.reshape(-1, 64, 64, 3)
    ...
    array, array_small = create_training_data_parallel(datadir)
    X1 = array.reshape(-1, 128, 128, 3)
    Xs1 = array_small.reshape(-1, 64, 64, 3)
    ...
    train_model_parallel(X_train, y_train, generator, discriminator, gan_model, vgg)
